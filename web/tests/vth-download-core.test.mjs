...
test("assembles the Windows v1.44.0 ZIP from its default manifest", async () => {
  const sample = fixture({
    fileName: "vth-similarity-windows-x64.zip",
    manifestPath: "/downloads/windows-package-v1.44.0.json",
    partPath:
      "/downloads/chunks/vth-similarity-windows-x64-v1.44.0.zip.part-000",
  });
  ...
  assert.deepEqual(sample.requests, [
    "/downloads/windows-package-v1.44.0.json",
    sample.manifest.parts[0].path,
  ]);
});

test("assembles the Ubuntu Universal x64 + ARM64 v1.44.0 tar.gz", async () => {
  const sample = fixture({
    fileName: "vth-similarity-ubuntu-universal.tar.gz",
    manifestPath: "/downloads/ubuntu-package-v1.44.0.json",
    partPath:
      "/downloads/chunks/vth-similarity-ubuntu-universal-v1.44.0.tar.gz.part-000",
  ...
  assert.deepEqual(sample.requests, [
    "/downloads/ubuntu-package-v1.44.0.json",
    sample.manifest.parts[0].path,
  ]);
});

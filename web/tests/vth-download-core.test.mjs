...
test("assembles the Windows v1.42.0 ZIP from its default manifest", async () => {
  const sample = fixture({
    fileName: "vth-similarity-windows-x64.zip",
    manifestPath: "/downloads/windows-package-v1.42.0.json",
    partPath:
      "/downloads/chunks/vth-similarity-windows-x64-v1.42.0.zip.part-000",
  });
  ...
  assert.deepEqual(sample.requests, [
    "/downloads/windows-package-v1.42.0.json",
    sample.manifest.parts[0].path,
  ]);
});

test("preserves the Ubuntu tar.gz format declared by its v1.42.0 manifest", async () => {
  const sample = fixture({
    fileName: "vth-similarity-ubuntu-x64.tar.gz",
    manifestPath: "/downloads/ubuntu-package-v1.42.0.json",
    partPath:
      "/downloads/chunks/vth-similarity-ubuntu-x64-v1.42.0.tar.gz.part-000",
  });
  ...
  assert.deepEqual(sample.requests, [
    "/downloads/ubuntu-package-v1.42.0.json",
    sample.manifest.parts[0].path,
  ]);
});

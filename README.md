...
`artifacts/windows/vth-similarity-windows-x64-v1.30.0.zip`은 공식
...
사진성 블록을 섞은 고해상도 2장과 저해상도 1장을 포함합니다. 화면의
`랜덤 멀티 차트 분석`은 이 세 샘플 중 직전과 다른 이미지를 골라 실행하며,
세 다운로드 링크로 각 원본을 받을 수 있습니다. 패키지 검증기는 모든
임의 배치 샘플에서 차트만 남고 비차트 후보가 제외되는지 확인합니다.

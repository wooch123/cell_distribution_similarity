...
구제 전처리를 채택합니다.
한 이미지에 서로 다른 좌표의 차트가 여러 개 있으면 직사각형 프레임과
열린 L축을 좌표별로 검출해 겹치지 않는 패널로 분리합니다. 각 패널은 기존
...
한 이미지에서 최대 24개 차트를 좌표별로 분리하며, 24개를 초과하면
신뢰도가 높은 24개를 선택한 뒤 행 우선 순서로 반환합니다. 패키지에는
3행×4열의 12차트 PPT 샘플 PNG도 포함되어 화면에서 즉시 분석하거나
파일로 내려받을 수 있습니다. 샘플의 예상 State 배열은
`4·8·8·8 / 4·8·8·8 / 4·8·8·8`이며, 패키지 검증기가 모든 내부 플롯의
검출 State와 관측 State가 이 배열과 일치하는지 확인합니다.

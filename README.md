...
패널로 분리합니다. 입력 가로·세로 픽셀 수를 차트 판정의 고정 최소·최대로
사용하지 않으며, 저해상도 입력은 제한된 작업 메모리 안에서 최대 16배로
...
단계에서만 사용하고 정규화 뒤 유사도 특징에서는 제외합니다. 기존 연동용
대표 `profile/query/results`는 가장 비정규적인 시리즈를 계속 가리킵니다.
축 모양의 사각형이 있더라도 내부에 연속성·곡률·수직 변화와 peak/tail
근거를 갖춘 분포 파형이 없으면
설명 텍스트·표·빈 좌표계·사각형·순서도·사진 영역으로 판정해 검색과
학습에서 제외합니다. API는 제외 건수를 `rejectedNonChartCount`로
반환하며 이미지 전체에 유효한 분포 파형이 없으면
`422 distribution_waveform_not_found`를 반환합니다. 기존 오류 코드는
호환성을 위해 유지하고, `reasonCode`와 `details`에는
`VTH-DETECT-NO-FOREGROUND`, `VTH-DETECT-LOW-RESOLUTION`,
`VTH-DETECT-TABLE-LATTICE`, `VTH-DETECT-NO-WAVEFORM`,
`VTH-DETECT-CANDIDATES-REJECTED`,
`VTH-DETECT-CANDIDATES-AMBIGUOUS` 중 해당 원인, 권장 조치, 입력/분석
해상도와 후보·제외 통계를 함께 반환합니다. 화면도 같은 정보를 기존 오류
영역에 표시하고 원인별 코드 안내를 접어서 제공합니다.
...
`artifacts/windows/vth-similarity-windows-x64-v1.47.0.zip`은 공식
...
LAN IP나 사내 DNS의 일반 HTTP 접속에서는 `/api/v1/runtime` 계약으로
서버판을 판별하고, 브라우저에 `crypto.randomUUID()`가 없어도
`crypto.getRandomValues()` 기반 RFC 4122 UUID v4를 생성합니다. 따라서
검색·랜덤 데모·붙여넣기·로컬 학습·일괄 학습·데이터 관리가 동일하게
동작하며, x64 CI는 실제 비루프백 HTTP의 headless Chrome으로 이를
검증합니다.
...
물리 프레임 증거를 검증해 유효한 분포 Curve를 보존합니다.
v1.42.0은 하나의 차트 내부에 촘촘한 가이드 격자가 있어 표처럼 보이는
경우에도, 후보 영역의 물리 격자와 직선 제거 후 남은 파형의 연속성을 함께
검증해 실제 분포만 복구합니다. 한 차트의 독립적인 전체 폭 색상 분포가
1~2개면 각각 검색·학습 대상으로 유지하고, 3개 이상이면 모든 색상 분포의
비정규성 점수를 비교해 가장 비정규적인 산포 하나만 사용합니다. 단일
봉우리 색상도 독립 분포 후보와 비정규성 비교 대상에 포함됩니다.
v1.41.0은 800×450 이미지의 절반 영역에 2~16개 분포 차트가 표처럼
밀집한 배치와, 4×4 차트가 물리 경계를 공유해 전역 표 격자로 보이는
배치를 추가로 처리합니다. 전역 격자 판정만으로 셀을 제거하지 않고,
각 셀의 실제 픽셀에서 2개 이상 peak와 그 사이 valley를 다시 측정합니다.
복수 셀이 이 peak–valley 위상 검증을 통과하고 검출 행·열 수가 물리 격자와
일치할 때만 차트 묶음으로 복구합니다. 저품질 JPEG, 회전 표, 단일 Gaussian
스파크라인 표는 같은 경로를 통과하지 못하도록 고정 회귀로 검증합니다.
...
`/downloads/ubuntu-package-v1.47.0.json`을 고정 매니페스트 경로로
